...
interface UseErrorLoggingReturn {
  ...
  simulateError: (message: string, severity?: string, category?: string, tenantId?: string) => Promise<any>;
...
export const useErrorLogging = (options: UseErrorLoggingOptions = {}): UseErrorLoggingReturn => {
  ...
  const resolveError = useCallback(async (errorId: string, resolutionData: ErrorResolutionRequest) => {
    try {
      ...
      // Update local state
      safeSetState(setActiveErrors, (prev: any) => 
        prev.map((error: any) => 
          error.id === errorId 
      ...
      if (enableNotifications) {
        ...
        safeSetState(setNotifications, (prev: any) => [notification, ...prev]);
      }
    } catch (error) {
  ...
  const handleErrorUpdate = useCallback((data: any) => {
    ...
    safeSetState(setActiveErrors, (prev: any) => {
      const existingIndex = prev.findIndex((error: any) => error.id === data.id);
      if (existingIndex >= 0) {
  ...
  const handleConnectionStateChange = useCallback((connected: boolean, error?: string) => {
    safeSetState(setConnectionState, (prev: any) => ({
      ...prev,
  ...
  const connectRealTime = useCallback(() => {
    safeSetState(setConnectionState, (prev: any) => ({ ...prev, isConnecting: true }));
  ...
  const markNotificationAsRead = useCallback((notificationId: string) => {
    safeSetState(setNotifications, (prev: any) => 
...
export const useErrorLog = (errorId: string) => {
  const { activeErrors, isLoadingErrors, error } = useErrorLogging();
  return { 
    data: activeErrors.find(e => e.id === errorId),
...
export const useResolveError = () => {
  const { resolveError } = useErrorLogging();
  return { 
    mutate: async (data: any) => resolveError(data.errorId, data.resolutionData),
    mutateAsync: async (data: any) => resolveError(data.errorId, data.resolutionData),
    isPending: false
  };
};
...
export const useDeleteError = () => {
  ...
  return { 
    mutate: async (errorId: string) => console.log('Delete error:', errorId),
    mutateAsync: async (errorId: string) => console.log('Delete error:', errorId),
    isPending: false
  };
};
...
export const useBulkErrorAction = () => {
  // Placeholder - implement if needed
  return { 
    mutate: async (data: any) => console.log('Bulk action:', data),
    mutateAsync: async (data: any) => console.log('Bulk action:', data),
    isPending: false
  };
};