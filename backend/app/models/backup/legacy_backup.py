"""
Legacy backup and recovery system models (preserved for backward compatibility)
"""
...
class BackupLog(BaseModel):
    """
    Backup operation log (legacy)
    """
    __tablename__ = "backup_logs"
    
    # Backup Information
    backup_type = Column(
        Enum(LegacyBackupType), 
        nullable=False,
        comment="Type of backup"
    )
    
    status = Column(
        Enum(LegacyBackupStatus), 
        default=LegacyBackupStatus.PENDING,
        nullable=False,
    ...
    def start_backup(self):
        ...
        self.status = LegacyBackupStatus.IN_PROGRESS
    ...
    def complete_backup(self, file_size: int = None, compressed_size: int = None, 
        ...
        self.status = LegacyBackupStatus.COMPLETED
    ...
    def fail_backup(self, error_message: str):
        ...
        self.status = LegacyBackupStatus.FAILED
    ...
    def cancel_backup(self):
        ...
        self.status = LegacyBackupStatus.CANCELLED
        self.completed_at = datetime.now(timezone.utc)
    ...
    def is_successful(self) -> bool:
        ...
        return self.status == LegacyBackupStatus.COMPLETED


class RestoreLog(BaseModel):
    """
    Restore operation log (legacy)
    """
...
class LegacyStorageLocation(BaseModel):
    """
    Storage location configuration (legacy)
    """
    __tablename__ = "storage_locations_legacy"
    ...
    def __repr__(self):
        return f"<LegacyStorageLocation(id={self.id}, name='{self.name}', provider='{self.provider.value}')>"
...
StorageLocation = LegacyStorageLocation
...
Index('idx_storage_location_provider', LegacyStorageLocation.provider)
Index('idx_storage_location_active', LegacyStorageLocation.is_active)
Index('idx_storage_location_primary', LegacyStorageLocation.is_primary)
